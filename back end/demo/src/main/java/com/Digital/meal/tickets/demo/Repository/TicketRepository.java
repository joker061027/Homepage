...
public interface TicketRepository extends JpaRepository<Ticket,String> {
    ...
    // 关键：通过 JOIN FETCH 依次关联 ticketType 和 canteen
    @Query("SELECT t FROM Ticket t " +
            "JOIN FETCH t.ticketType tt " + // 关联票种类型
            "JOIN FETCH tt.canteen c")      // 关联食堂表

    List<Ticket> findAllWithTicketTypeAndCanteen();

    @Query("SELECT t FROM Ticket t " +
            "JOIN FETCH t.ticketType tt " + // 关联票种类型
            "JOIN FETCH tt.canteen c " +    // 关联食堂表
            "WHERE t.tickerId = :id")
    Optional<Ticket> findByIdWithTicketTypeAndCanteen(@Param("id") String id);
    ...
    // 根据状态查询餐票
    List<Ticket> findByStaus(String staus);

    // 根据状态统计餐票数量
    long countByStaus(String staus);

    // 根据餐票编号查询（关键方法，用于check和use接口）
