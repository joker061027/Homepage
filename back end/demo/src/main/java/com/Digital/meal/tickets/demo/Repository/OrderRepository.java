...
public interface OrderRepository extends JpaRepository<OrderEntity, String> {
   ...
   @Query(nativeQuery = true, value =
    ...
    List<Object[]> findOrderSummaries();//首页餐票订单
    ...
    // 简化版本的订单汇总查询 - 只查询订单基本信息，避免复杂JOIN
    @Query(nativeQuery = true, value =
        "SELECT " +
        "  o.OrderNumber AS orderNumber, " +
        "  o.CreateDatetime AS createDatetime, " +
        "  0 AS totalOrderAmount, " +  // 暂时设为0，后续可以通过其他方式获取
        "  0 AS ticketStatus1Count, " + // 暂时设为0，后续可以通过其他方式获取
        "  o.Status AS orderStatus " +
        "FROM `order` o " +
        "ORDER BY o.CreateDatetime DESC")
    List<Object[]> findSimpleOrderSummaries();
    ...
    @Query(nativeQuery = true, value =
        ...
        "  0 AS ticketStatus1Count, " +
        "  o.Status AS orderStatus " +
        "FROM `order` o " +
        "WHERE o.OrderId IN :orderIds " +
        "ORDER BY o.CreateDatetime DESC")
    List<Object[]> findSimpleOrderSummariesByOrderIds(@Param("orderIds") List<String> orderIds);
}